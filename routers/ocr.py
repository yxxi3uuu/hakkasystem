...
# Avoid a PaddlePaddle/PaddleX oneDNN runtime bug on Windows CPU inference.
...
def _init_paddle():
    ...
    try:
        ...
        try:
            ...
            # PaddleOCR 2.x uses the older angle-classifier parameter names.
    ...
    for item in result or []:
        if isinstance(item, dict) and "rec_texts" in item:
            ...
            for text, conf in zip(texts, scores):
                print(f"[OCR] PaddleOCR line: {repr(text)} confidence={conf:.2f}")
                if text and conf >= 0.5:
                    lines.append(text)
            continue

        page = item if item else []
        for line in page:
...
def _run_paddle(image_path: str) -> str:
    ...
    try:
        ...
        if result and result[0] and not isinstance(result[0], dict):
